...
errorLoggingQuery =   "INSERT INTO errorLog (traceBack,information,type)"\
                      " VALUES('%s','%s','%s')"
commit =              "commit"
# SQL queries used in the system
...
def slotList():
   # return the list of Slots. Currently Slots are of 1 hour
   # and multiple of integer
   type1Slot='0%s:00'
   # pattern for slots from 00:00 to 09:00

   type2Slot='%s:00'
   # pattern for slots from 10:00 to 23:00
...
def flightPrefix(orig,dest):
   # returns the flight prefix that is orig+dest
   # for e.g 'DEL'+'BLR' = 'DELBLR'
   return orig+dest
...
def flightQuerySufix(yyyymmdd):
  # creates suffix of search pattern from date
  # %Y%M%D 
  yyyymmdd = yyyymmdd.replace('/',' ')
  yyyymmdd = yyyymmdd.replace('-',' ')
  yyyymmdd = yyyymmdd.split()
...
def flightSearchPattern(orig,dest,date):
   # returns search pattern for flight.
   # 'PREFIX%SUFIX'
...
#def connection(cnx=createConnection()):
def connection():
...
def flightDescription(code,timimngs):
   ...
   pair2 += flightDescriptionPair2(\
              cityNames([city1,city2,city3,city4]),\
              sufix)
   return (pair1,pair2) 
